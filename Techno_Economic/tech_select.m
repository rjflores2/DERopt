...
if pv_on
    ...
    pv_v=[pv_cap_cost; 0.2 ; 0.001];  %https://atb.nrel.gov/electricity/2021/residential_pv
    ...
    pv_fin = [-0.4648; ... %%%Scaling linear factor - Based on Lazards cost of electricity
        pv_macrs; ... %%%MACRS Schedule Modified Accelerated Cost Recovery System (tax) 2025:5__2030:0__2045:0
        pv_itc]; ... %%%ITC Solar Investment Tax Credit (ITC)2025:1__2030:0__2045:0   
...
if ees_on
    ...
    ees_v=[ees_cap_cost; 0.001; 0.001; 0.1; 0.95; 0.5; 0.5; .90; .90; .9999];
    ees_cap=ees_v(1);
    ...
    ees_fin = [-0.4648;... %%%Scaling linear factor - Based on Lazards cost of electricity ($/kW installed)
        ees_macrs; ... %%%MACRS Schedule:Modified Accelerated Cost Recovery(tax depreciation system to calculate asset depreciation)-PPA(Power Purchase Agreement) 2025:7__2030:0__2045:0   
        ees_itc]; ... %%%ITC Benefit:Investment Tax Credit(federal tax incentive for business investment) 2025:0__2030:0__2045:0  
        
    %%%Financial Aspects - EES
    rees_fin = [-0.4648;... %%%Scaling linear factor - Based on Lazards cost of electricity
        rees_macrs; ... %%%MACRS Schedule 2025:5__2030:0__2045:0   
        rees_itc]; ... %%%ITC Benefit 2025:1__2030:0__2045:0 
...
if erwh_on
    erwh_v = [0    %%% 1000  1: Capital cost ($/kWel) https://www.homedepot.com/
               erwh_eff];   %CZ15:WHP COP:2.62 CZ16:ERWH:0.95 PremiumERWH:1 
...
if gwh_on
    gwh_v = [0    %%%2000 1: Capital cost ($/kWth) 
              gwh_eff];  %%% 2: GWH energy factor (EF) Baseline:0.6, Premium_Gas:0.96    
else 
    gwh_v = [];
end
...
if gsph_on
    gsph_v = [0    %%%3000 1: Capital cost ($/kWth)- 75,000–100,000 BTU: $2,500–$5,900: An 80,000 BTU furnace will keep a 1,600- to 2,000-square-foot home warm
              gsph_eff];  %%% BEopt: Baseline: 0.8 - Premium Gas: 0.96  
else 
    gsph_v = [];
end 
...
if ersph_on
    ersph_v = [0    %%%1200 1: Capital cost ($/kWel)-  
              ersph_eff];  %%%1 for ERSPH, 2 for COP_HPSPH = 2    2: 100%  energy efficient-https://www.energy.gov/energysaver/electric-resistance-heating 
else 
    ersph_v = [];
end 
%% SGIP incentives
