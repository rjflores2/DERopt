...
if isempty(ees_v) == 0
    for k=1:K
        ...
        if isempty(pv_v) == 0 && rees_on
            %%%SOC Equality / Energy Balance
            Constraints = [Constraints
                rees_dchrg_nem(1,k) == 0
                rees_soc(2:T,k) == ees_v(10)*rees_soc(1:T-1,k) + ees_v(8)*rees_chrg(2:T,k)  - (1/ees_v(9))*(rees_dchrg(2:T,k) + rees_dchrg_nem(2:T,k))  %%%Minus discharging of battery
