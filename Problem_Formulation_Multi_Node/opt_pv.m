...
if isempty(pv_v) == 0
    ...
    if curtail
        Constraints = [Constraints, (var_pv.pv_elec + var_pv.pv_nem + var_rees.rees_chrg <= (1/e_adjust).*repmat(solar,1,K).*(repmat(pv_legacy(2,:),T,1) + repmat(var_pv.pv_adopt,T,1))):'PV Energy Balance'];
