...
if ~isempty(rsoc_v)
    Constraints = [Constraints
        ...
        (-var_rsoc.rsoc_adopt.*rsoc_v(6) <= var_rsoc.rsoc_elec(2:size(var_rsoc.rsoc_elec,1)) - ...
            var_rsoc.rsoc_elec(1:size(var_rsoc.rsoc_elec,1) - 1) <= var_rsoc.rsoc_adopt*rsoc_v(6)):...
            'rSOC electricity Production'
        (-var_rsoc.rsoc_adopt.*rsoc_v(6) <= var_rsoc.rsoc_prod(2:size(var_rsoc.rsoc_prod,1)) - ...
            var_rsoc.rsoc_prod(1:size(var_rsoc.rsoc_prod,1) - 1) <= var_rsoc.rsoc_adopt*rsoc_v(6)):...
        ...
        (-(1-var_rsoc.rsoc_bin)*1000 + var_rsoc.rsoc_adopt*rsoc_v(6) <= var_rsoc.rsoc_elec):'rSOC min load constraint'
        (-(1-var_rsoc.rsoc_bin)*1000 + var_rsoc.rsoc_adopt*rsoc_v(7) <= var_rsoc.rsoc_elec):'rSOC ramp limit constraint'];
