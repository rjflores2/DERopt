...
if isempty(ees_v) == 0
    for ii = 1:size(ees_v,2)
        ...
        Constraints = [Constraints
            ...
            (var_ees.ees_chrg(:,ii) <= var_ees.ees_bin(:,ii)*10000):'EES Chrg Op State'
            (var_ees.ees_dchrg(:,ii) <= (1 - var_ees.ees_bin(:,ii))*10000):'RRES Dchrg Op State']; 
        %% Renewable Tied EES
        if isempty(pv_v) == 0 && rees_on && rees_op_state
            %%%SOC Equality / Energy Balance
            Constraints = [Constraints
                ...
                (var_rees.rees_chrg(:,ii) <= var_rees.rees_bin(:,ii)*10000):'REES Chrg Op State'
                (var_rees.rees_dchrg(:,ii) <= (1 - var_rees.rees_bin(:,ii))*10000):'RRES Dchrg Op State'];
