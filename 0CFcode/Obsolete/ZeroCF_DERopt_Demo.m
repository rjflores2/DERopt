...
for ii = 1:length(cfg.co2_red)
    ...
    % Define week to plot
    %
    %   If  cfg.month_idx = 7  ==> graphIndex = 1
    %   If  cfg.month_idx = [1 4 7 10]  ==> graphIndex = 3

    graphIndex = find(cfg.month_idx == 7);
    ...
    grid on
    datetick('x','ddd','keepticks')
    xlim([bldgData.time(bldgData.stpts(graphIndex)) bldgData.time(bldgData.stpts(graphIndex)+96*7)])
